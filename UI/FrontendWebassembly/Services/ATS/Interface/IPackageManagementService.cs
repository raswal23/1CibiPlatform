...
{
	Task<PaginatedResult<PackageDetailsDTO>> GetPackagesAsync(int? PageNumber = 1, int? PageSize = 10, string? SearchTerm = null, CancellationToken cancellationToken = default);
	Task<IReadOnlyList<PackageDetailsDTO>> GetAllPackagesAsync(CancellationToken cancellationToken = default);
