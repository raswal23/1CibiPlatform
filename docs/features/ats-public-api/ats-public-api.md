...
The database side is documented separately in **`docs/operations/package-id-migration.md`**, which
...
7. **The migration assumes empty order tables.** `PackageId` is added `NOT NULL` with a
   ...
   applying it elsewhere must read `docs/operations/package-id-migration.md` first, which has the
