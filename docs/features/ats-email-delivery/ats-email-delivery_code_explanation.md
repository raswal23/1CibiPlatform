...
`BuildMessage` also takes an optional copy list and adds each address to `message.Cc`. Only the three
order notices pass one — withdrawal, dispute and completed-form, in
`docs/features/ats-withdrawn-application-email/`, `docs/features/ats-dispute-order-email/` and
`docs/features/ats-submitted-form-email/`. Every other path sends to a single recipient, exactly as
before. The completion notice copies three addresses, so it charges the sending account four
recipients against the daily cap.
...
|---|---|---|---|
| `SendATSEmailWithResultAsync` | `ATSEmailService.cs:61` | `EndorsementSubmissionService.SendApplicationFormToUserEmailWithResultAsync`; `WithdrawnEmailNotification.SendNoticeAsync`; `DisputeEmailNotification.SendNoticeAsync`; `SubmittedFormEmailNotification.SendNoticeAsync` | The traced path. The switcher. The three order notices are the only callers that pass `cc` |
| `SendATSEmailAsync` → `bool` | `:27` | `SendEmailAsync` (`:558`), `EndorsementSubmissionService.cs:430` | Wraps the switcher and **discards the outcome**: `var result = await SendATSEmailWithResultAsync(toEmail, subject, body, CancellationToken.None); return result.IsSent;` — note `CancellationToken.None`, so this path cannot be cancelled at all |
