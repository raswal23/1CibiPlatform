...
**Fix.** Derive the group from the authenticated principal rather than the query string:

> **Correction (post-implementation).** This section originally also recommended
> `.RequireAuthorization()` on the `MapHub` call. That advice was wrong for this codebase:
> the client connects with a bare `HubConnectionBuilder`, which does not go through
> `CookieHandler`, so no auth cookie is sent cross-origin on the handshake — every
> connection 401s, taking down New Order and the AI assistant. The group fix below closes
> the hole on its own.
