...
## Correction: `[Authorize]` was added, then removed

I first put `[Authorize]` on both hubs as defence in depth. **That broke New Order and the
AI assistant with a 401** and was reverted.

Why it broke: the auth cookie never reaches the hub. `CookieHandler` is what sets
`BrowserRequestCredentials.Include`, and it is registered on the *named* HttpClients
(`"API"`, `"RefreshAPI"`, `"SSOAPI"`). The hub uses a bare builder:

```csharp
_hubConnection = new HubConnectionBuilder()
    .WithUrl(hubUrl)          // no CookieHandler, no credentials
    .WithAutomaticReconnect()
    .Build();
```

UI and API are different origins (`localhost:5134` vs `localhost:5123`), so the browser
sends no cookie on the handshake → no principal → `[Authorize]` 401s every connection.
`NewOrderComponent.OnInitializedAsync` awaits `StartAsync()`, so the throw took the whole
page down.

**Why no test caught it:** `UseSignalRConfiguration` early-returns under the `Testing`
environment, so no test exercised a hub. Build green, 547 tests green, app broken.

**What actually closes the vulnerability is the group fix alone.** A connection with no
principal gets `null` from `GetUserGroupName()`, joins no group, and receives nothing.
`[Authorize]` was a second lock added without checking anyone still had the key.

The reasoning is recorded in the hub so it is not re-added blindly:
...
To add it back later, the client needs a credentialed handler on the connection
(`options.HttpMessageHandlerFactory`) — a deliberate change, tested against a running app
rather than a build.

## Closing the blind spot: `AtsHubGroupIsolationTests`

The hub had **no test coverage at all**, which is what let both the original vulnerability
and my `[Authorize]` regression through.

Rather than change the `Testing` early-return (which would alter how every other
integration test boots), the new suite stands up a minimal host containing only the hub
and a stub auth handler — no database, no Testcontainer. The subject under test is the
group-assignment rule; nothing else needs to be real:
        ...
        webBuilder
            ...
            .ConfigureServices(services =>
            {
                ...
                services.AddAuthentication(StubAuthHandler.SchemeName)
                    .AddScheme<StubAuthOptions, StubAuthHandler>(StubAuthHandler.SchemeName, _ => { });
...
| Test | Asserts |
|---|---|
| `Connection_ShouldReceiveMessagesForItsOwnUser` | Baseline delivery works |
| `Connection_ShouldNotReceiveAnotherUsersMessages` | The finding itself |
| `Connection_ShouldIgnoreAUserIdSuppliedInTheQueryString` | The exact old attack — `?userId=<victim>` has no effect |
| `Connection_WithNoIdentity_ShouldJoinNoGroupAndReceiveNothing` | Why `[Authorize]` is unnecessary |
| `TwoConnectionsForTheSameUser_ShouldBothJoinThatUsersGroup` | Two tabs both work |

### Verified by reintroducing both bugs

A test that has never failed proves nothing, so both regressions were re-applied and the
suite re-run:

| Regression re-applied | Result |
|---|---|
| Original vulnerability (`GetUserGroupName()` → `Request.Query["userId"]`) | **5 of 5 failed** |
| My `[Authorize]` regression | `Connection_WithNoIdentity_...` failed in 425 ms — the same 401 that took down New Order |

Both were then reverted; the suite is green.

### One test deliberately dropped

A reconnect-after-stop test was written and removed. Under TestServer with the
`LongPolling` transport, a connection that has been stopped suppresses group delivery to
*subsequent* connections in the same host — while two concurrently live connections work
fine. That is a harness artifact, not hub behaviour, so asserting through it would test
the transport rather than `ATSHub`. The reasoning is recorded as a comment where the test
would have been, so nobody re-adds it and spends the same hour.
...
// principal - a connection with no principal joins no group.
