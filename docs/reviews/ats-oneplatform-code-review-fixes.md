...
| 2 | SignalR hub joined any group named in the query | Group from `Context.User` | +5 |
...
> **`[Authorize]` was added here and then removed.** It 401'd every hub connection — the
> client uses a bare `HubConnectionBuilder` that never goes through `CookieHandler`, so
> the auth cookie is not sent cross-origin on the handshake. That broke New Order and the
> AI assistant. The group fix alone closes the vulnerability: no principal → no group →
> no messages. `AtsHubGroupIsolationTests` now covers both regressions. See
> [ats-oneplatform-fix-details.md](ats-oneplatform-fix-details.md).
