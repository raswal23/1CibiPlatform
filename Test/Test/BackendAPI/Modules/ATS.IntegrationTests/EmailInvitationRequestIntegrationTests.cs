...
public class EmailInvitationRequestIntegrationTests : BaseIntegrationTest
{
	public EmailInvitationRequestIntegrationTests(IntegrationTestWebAppFactory factory) : base(factory)
	{
	...
	public async Task InsertEmailInvitationRequest_ShouldReturnCreatedIdAndPersist()
	{
		...
		result.EmailInvitationID.Should().NotBe(Guid.Empty);

		var persisted = await _dbContext.EmailInvitationRequests.FindAsync(result.EmailInvitationID);
